...
export const taskService = {
  createTask(input: CreateTaskInput): Task {
    ...
    const providedTitle = stripInvisible(input.title ?? "").trim();
   ...
   * Renames a Task.
   *
   * Titles are generated by a heuristic (see `generateTitleFromInstruction`),
   * and a heuristic is occasionally wrong about what a Task was for — which
   * used to be permanent, because a title could only ever be set at creation
   * and the web form did not even offer the field. This is the correction
   * path; it is the *only* thing it changes, and it is allowed in every
   * status, including terminal ones, since naming past work correctly is
   * exactly when you discover the name is wrong.
   *
   * An empty title falls back to regenerating from the instruction rather
   * than storing a blank, so a Task always has something to be listed under.
   */
  renameTask(identifier: string, rawTitle: string): Task {
    const task = requireTask(identifier);
    // Zero-width and format characters survive trim(), so a title made only of
    // them would pass an "is it non-empty" check and store a Task that renders
    // as a blank row — defeating the fallback below.
    const trimmed = stripInvisible(rawTitle ?? "").trim();
    const title = trimmed || generateTitleFromInstruction(task.instruction);
    if (title === task.title) return task;

    const updated = taskStore.update(task.id, { title });
    if (updated) {
      emitSystemLog(task.id, `제목을 "${title}"(으)로 변경했습니다.`);
    }
    return updated ?? task;
  },
